...
OrkMesh::OrkMesh(ptr< Mesh<V_UV_N_T_B, unsigned int> > mesh, Material material, AABB boundAABB, OctreeNode *root, 
	...
	: mesh(mesh), material(material), boundAABB(boundAABB), root(root),
...
void ObjMesh::CreateOctree(OctreeNode *root)//计算八叉树
{
	root->boundFaces = faces;
	root->aabb = boundAABB;

	OctreeManager::CreateOctree(root);
}
